...
Double_t dNdPEMF(Double_t PE,Double_t nucleusM,Double_t Nnu,Double_t NTtotal,Double_t s);
Double_t Nu3MF(Double_t nucleusM,Double_t Nnu,Double_t NTtotal,Double_t s);
Double_t pdg()
{
    Double_t sum;
    Double_t Nnu = 1.76e23*0.08/(4*PI*pow(L,2));
    struct datapointM a;
    struct rawdataM b;
    ...
    /*error bar*/
    auto c1 = new TCanvas("c1","Asymmetric error bars",200,10,700,500);
    c1->SetFillColor(0);
    c1->SetGrid();
    ...
    gr->GetXaxis()->CenterTitle("Number of photoelectrons (PE)");
    gr->GetYaxis()->CenterTitle("Res. counts / 2 PE");
    gr->Draw("AP");
    ...
    for (Int_t i=0; i<n1; i++) 
    {
        p[i] = i*0.1;
        ErkeV = p[i];
        q[i] = log10(dNdErMF(ErkeV));
    }
    //create graph
    TGraph *gr1 = new TGraph(n1,p,q);
    TCanvas *c2 = new TCanvas ("c2","Recoil Energy",
    800,600,800,600);
    gr1->GetXaxis()->CenterTitle("Recoil energy (keV)");
    gr1->GetYaxis()->CenterTitle("Counts");
    // draw the graph with the axis,contineous line, and put
    c2->cd(4);
    gr1->Draw("AL");
    sum = hbar + Mamu;
    std::cout << q[699] << std::endl;
    std::cout << sum << std::endl;
    std::cout << Enumu << std::endl;
    return 0;
}
...
/*10^(-38)cm^2/MeV*/
{
    Double_t QwI = ZNuc*gpV*FF(sqrt(2*MNuc*Er),Rprms,s)+NNuc*gnV*FF(sqrt(2*MNuc*Er),Rnrms,s);
    Double_t res = GF^2/(2*PI)*MNuc*(2-MNuc*Er/(Enu^2))*pow(hbar,2)*(pow(QwI,2)/4);
    return res;
}
Double_t dNnumudErF(Double_t ZNuc,Double_t NNuc,Double_t MNuc,Double_t Er,Double_t Rprms,Double_t Rnrms,Double_t s)
{
...
/*Er MeV*/
{
    ...
    sum = ((Cs + I)e-23)*Nnu*NTtotal/1000;
    return sum;
}